...
fn is_steam_deck() -> bool {
    if let Ok(product) = std::fs::read_to_string("/sys/class/dmi/id/product_name") {
        ...
        if product.contains("jupiter") || product.contains("galileo") || product.contains("steam deck") {
            return true;
        }
    }
...
/// Override esplicito: MHFZ_GRAPHICS_MODE=hd|sd.
...
fn proton_path_in_library(library_root: &str) -> Option<PathBuf> {
    for proton_name in ["Proton - Experimental", "Proton Experimental"] {
        let proton_path = PathBuf::from(library_root)
...
fn find_proton_experimental() -> Option<PathBuf> {
    ...
    for steam_root in &steam_roots {
        if let Some(proton_path) = proton_path_in_library(steam_root) {
    ...
    for steam_root in &steam_roots {
        ...
        if let Ok(content) = std::fs::read_to_string(&vdf_path) {
            for line in content.lines() {
                ...
                if let Some(proton_path) = proton_path_in_library(&path_str) {
...
/// Rileva l'esecuzione nel container Distrobox.
/// Serve soltanto nel workflow di sviluppo: Proton deve girare sull'host SteamOS.
...
/// Escape per valori inseriti in una stringa bash con apici singoli.
...
/// Proton e' il runtime preferito per entrambe le modalita', SD e HD.
fn detect_wine_runtime(mode: GraphicsMode) -> WineRuntime {
    if let Some(proton_path) = find_proton_experimental() {
        log_to_file(&format!(
            "🟣 Runtime selezionato: Proton Experimental ({:?})",
                             mode
        ));
    ...
    if mode == GraphicsMode::Hd {
        log_to_file("⚠️ Proton Experimental non trovato per HD, fallback a Wine");
    }
    ...
    if let Ok(entries) = std::fs::read_dir(&fonts_source) {
        for entry in entries.flatten() {
            ...
            if let Some(filename) = path.file_name() {
                ...
                let is_allowed = allowed_fonts
                .iter()
                .any(|&allowed| filename_str == allowed.to_lowercase());
    ...
    for font_file in font_files {
        ...
        let status = match &runtime {
            WineRuntime::ProtonExperimental(proton_path) => {
                ...
                // wineprefix qui e' il path compatdata Proton, non il pfx interno.
                Command::new("python3")
                .arg(proton_path)
                .arg("run")
                .arg("reg")
            ...
            WineRuntime::WineFlatpak => Command::new("flatpak")
            .arg("run")
            .arg("--filesystem=home")
            ...
            .stderr(Stdio::null())
            .status(),
            WineRuntime::WineSystem => Command::new("wine")
            .arg("reg")
            .arg("add")
            .arg("HKLM\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Fonts")
            .arg("/v")
...
pub fn run_linux(cfg: MhfConfigLinux) -> std::io::Result<()> {
    ...
    let notices_json: Vec<serde_json::Value> = cfg
    .config
    .notices
    .iter()
    .map(|n| serde_json::json!({ "flags": n.flags, "data": &n.data }))
    .collect();

    let friends_json: Vec<serde_json::Value> = cfg
    .config
    .friends
    .iter()
    .map(|f| serde_json::json!({ "cid": f.cid, "id": f.id, "name": &f.name }))
    .collect();

    let mez_stalls_str: Vec<String> = cfg
    .config
    .mez_stalls
    .iter()
    .map(|s| format!("{:?}", s))
    ...
    std::fs::write(&config_path, serde_json::to_string_pretty(&config_json).unwrap()).map_err(|e| {
    ...
    let fontconfig_file =
    env::var("FONTCONFIG_FILE").unwrap_or_else(|_| "/etc/fonts/fonts.conf".to_string());
    let xdg_data_dirs =
    ...
    let (wineprefix, compat_data_path) = match &runtime {
        ...
        _ => {
            let pfx = env::var("WINEPREFIX")
            .unwrap_or_else(|_| cfg.game_folder.join("pfx").to_string_lossy().to_string());
            (pfx, None)
        }
    };
...
shell_escape(cdp),
        ...
        // Con Proton wineprefix e' il pfx interno; il registry riceve il compatdata.
    ...
    let dll_overrides = match mode {
        GraphicsMode::Hd => {
            "xinput1_3=n,b;dinput=n,b;dinput8=n,b;winemenubuilder.exe=d;d3d9=n,b;d3d11=n,b;dxgi=n,b"
        }
...
shell_escape(cdp),
        ...
        WineRuntime::WineFlatpak => Command::new("setsid")
        .arg("flatpak")
        .arg("run")
        .arg("--branch=stable-24.08")
        .arg("--filesystem=home")
        .arg(format!("--env=WINEPREFIX={}", &wineprefix))
        .arg("--env=WINEDEBUG=-all")
        .arg(format!("--env=WINEDLLOVERRIDES={}", dll_overrides))
        .arg(format!("--env=FONTCONFIG_PATH={}", fontconfig_path))
        .arg(format!("--env=FONTCONFIG_FILE={}", fontconfig_file))
        .arg(format!("--env=XDG_DATA_DIRS={}", xdg_data_dirs))
        .arg(format!("--env=XAUTHORITY={}", xauthority))
        .arg("org.winehq.Wine")
        .arg(&mhf_iel_exe)
        ...
        .stderr(Stdio::null())
        .spawn(),
        WineRuntime::WineSystem => Command::new("setsid")
        .arg("wine")
        .arg(&mhf_iel_exe)
        .env("WINEPREFIX", &wineprefix)
        .env("WINEDEBUG", "-all")
