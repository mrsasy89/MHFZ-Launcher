...
export const backgroundUrl      = computed(() => storePrivate.background     ?? fallbackBackground);
export const cogUrl             = computed(() => storePrivate.cog            ?? fallbackCog);
export const capcomUrl          = computed(() => storePrivate.capcom         ?? fallbackCapcom);
export const launcherHeaderUrl  = computed(() => storePrivate.launcherHeader ?? fallbackLauncherHeader);
...
const UI_PREF_KEY = "ui_prefs_v1";

function loadUiPrefs() {
  try {
    const j = JSON.parse(localStorage.getItem(UI_PREF_KEY) || "{}");
    if (typeof j.sfxEnabled === "boolean") storePrivate.settings.sfxEnabled = j.sfxEnabled;
    if (typeof j.sfxVolume  === "number")  storePrivate.settings.sfxVolume  = j.sfxVolume;
  } catch (_) {/* ignore */}
}
...
export async function initStore() {
  ...
  // ensure defaults exist (in case backend doesn’t know them)
  if (storePrivate.settings.sfxEnabled === undefined) storePrivate.settings.sfxEnabled = false;
  if (storePrivate.settings.sfxVolume  === undefined) storePrivate.settings.sfxVolume  = 70;

  // load local UI prefs last (wins)
  loadUiPrefs();

  // assets
  storePrivate.background      = bust(data.background)       ?? null;
  storePrivate.cog             = bust(data.cog)              ?? null;
  storePrivate.capcom          = bust(data.capcom)           ?? null;
  storePrivate.launcherHeader  = bust(data.launcher_header)  ?? null;
}
...
export async function setCurrentEndpoint(currentEndpoint) {
  ...
  try {
    ...
    storePrivate.background     = bust(data.background);
    storePrivate.cog            = bust(data.cog);
    storePrivate.capcom         = bust(data.capcom);
    storePrivate.launcherHeader = bust(data.launcher_header);
  } catch (error) {
    ...
    storePrivate.launcherHeader = null;
    throw error;
  }
}
