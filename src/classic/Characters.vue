...
function onCreateClick() {
  playSelect();
  // Insert a single placeholder card instead of immediately creating a character.
  addPlaceholderCharacter();
  // Move the carousel highlight to the new placeholder at the end of the list.
...
function onStartClick() {
  ...
  // If the highlighted entry is the placeholder (id is null or the placeholder flag is set),
  // create a new character instead of selecting it.
  if (
    character.value.id === null ||
    character.value.placeholder
  ) {
    doCreateCharacter();
  } else {
    doSelectCharacter(character.value.id);
  }
}
  ...
  <div class="h-full w-full" ref="rootRef">
    ...
    <div class="flex justify-center mt-5 gap-3 pl-4">
      ...
      >
        ...
        >
          <span>{{ $t('options-character-label') }}</span>
